    ...
    <div className="flex flex-col h-full bg-[#080b10] text-[#f3f4f6] font-sans">
      <header className="flex justify-between items-center bg-[#080b10] sticky top-0 z-10 py-5 px-8 border-b border-white/5">
        <div className="flex items-center gap-6">
          <h2 className="text-xl font-bold text-white tracking-wide">Processes</h2>
        ...
        <div className="flex items-center gap-6">
           <RefreshCw size={18} className="text-[#9ca3af] cursor-pointer hover:text-white transition-colors" onClick={fetchProcesses} />
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-[1200px] mx-auto w-full space-y-6">
        <h1 className="text-2xl font-bold text-white tracking-wide mb-2">System Processes</h1>

        <div className="bg-[#181d28] border border-[#7f77dd]/20 rounded-lg flex flex-col h-full border-t-2 border-t-[#7f77dd]">
          <div className="p-6 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Activity className="text-[#7f77dd] w-6 h-6" />
              <h2 className="text-xl font-display font-black text-white tracking-wide">System Processes</h2>
            </div>
            ...
            <div className="flex items-center gap-6">
              <div className="relative">
                ...
                <input 
                  ...
                  className="bg-black/50 border border-white/10 rounded px-9 py-2 text-sm text-white font-mono placeholder-[#9ca3af] focus:outline-none focus:border-[#7f77dd] w-64 transition-all" 
                />
              </div>
              <div className="text-right">
                <div className="text-[9px] text-[#9ca3af] font-black uppercase tracking-widest font-mono text-right">STATUS</div>
                <div className="text-[#7f77dd] text-xs font-mono tracking-widest uppercase">LIVE MONITORING</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-x-auto">
            <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
              <thead className="bg-black/40 text-[10px] font-bold text-[#9ca3af] uppercase tracking-widest">
                <tr>
                  <th className="py-4 px-6 font-mono cursor-pointer hover:text-white">PID ↑↓</th>
                  <th className="py-4 px-6 font-mono cursor-pointer hover:text-white">NAME ↑↓</th>
                  <th className="py-4 px-6 font-mono cursor-pointer hover:text-white">CPU % ↑↓</th>
                  <th className="py-4 px-6 font-mono cursor-pointer hover:text-white">MEMORY % ↑↓</th>
                  <th className="py-4 px-6 font-mono cursor-pointer hover:text-white">STATUS ↑↓</th>
                  <th className="py-4 px-6 font-mono cursor-pointer hover:text-white w-full">PATH ↑↓</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5 font-mono text-[#f3f4f6]">
                {filtered.map((proc, i) => (
                  <tr key={`${proc.pid}-${i}`} className="hover:bg-white/5 transition-colors cursor-default">
                    ...
                    <td className="py-3 px-6 text-[#9ca3af] text-xs truncate max-w-[300px]" title={proc.path}>{proc.path || '-'}</td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                   <tr><td colSpan={6} className="text-center py-8 text-[#9ca3af] font-mono text-sm">No processes found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
